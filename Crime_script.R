...
# vector to reorder x-axis
level_order <- c('2-4', '5-12', '13-19','20-39','40-59','60+')
data %>% 
  select(age_group,vict_sex,date_occured) %>% 
  filter(age_group != "Unkown",
         vict_sex != "X",
         vict_sex != "H",
         vict_sex != "",
         between(date_occured,'2020-01-01','2022-01-01')) %>%
  group_by(age_group,vict_sex) %>% 
  count(age_group) %>%
  ggplot(aes(x=factor(age_group,levels=unique(level_order)),y=n,fill=vict_sex)) +
  geom_histogram(stat = "identity",position = "dodge") +
  geom_text(aes(label=n),position=position_dodge(1),vjust=-0.5) +
  labs(title = "Age Group And Sex (2020-2022)",
       subtitle="Victims Of Crime By Age Group And Sex",
       x="Age Group", y="Count Of Victims",
       fill="Sex",
       caption="Data Provided by Los Angeles Police Department")

# Victims by descent and sex
data %>% 
  select(vict_sex,vict_descent,date_occured) %>% 
  filter(between(date_occured,'2020-01-01','2022-01-01'),
                 vict_sex != "X",
                 vict_sex != "H",
                 vict_sex != "",
         vict_descent != "",
         vict_descent != "Uknown") %>% 
  group_by(vict_sex,vict_descent) %>% 
  count(vict_descent) %>% 
  ggplot(aes(x=factor(vict_descent),y=n,
             fill=vict_sex)) +
  geom_histogram(stat = "identity",position = "dodge") +
  geom_text(aes(label=n),position=position_dodge(1),vjust=-0.5,size=4) +
  labs(title = "Descent and Sex (2020-2022)",
       subtitle="Victims Of Crime By Descent And Sex",
       x="Age Group", y="Count Of Victims",
       fill="Sex",
       caption="Data Provided by Los Angeles Police Department") +
